...
public:
  ...
  MarkerDetectorNode(ros::NodeHandle& nh, ros::NodeHandle& nh_private) : 
    nh_(nh), nh_private_(nh_private), it_(nh_), previous_marker_detected_(false)
  {
    ...
    if (lazy_)
    {
      connect_cb = boost::bind(&MarkerDetectorNode::connectCb, this);
    }
    else
    {
...
} // end namespace pattern_pose_estimation
